...
It opens a GUI control panel backed by `launcher/bogos-control-panel.ps1`.

The control panel:
- locates the project root
- checks for Node/npm
- checks for `ffmpeg` and `yt-dlp`
- checks `.env`, `node_modules`, `package.json`, and `src/index.js`
- runs `npm start`
- runs `npm run deploy`
- runs `npm run check`
- starts, stops, and restarts the bot
- shows live stdout/stderr logs
- writes launcher logs to `logs/bogos-launcher.log`
- supports optional crash auto-restart with a loop guard
